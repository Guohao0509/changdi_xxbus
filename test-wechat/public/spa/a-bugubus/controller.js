 ...
 **/
    ...
    .controller('ScheduleTicketPay',function($rootScope,$scope,$location,$state,$stateParams,$myHttpService,$filter){
        ...
        function getMonthDate(yy, mm) {
            ...
            date.remainDays = getWorkDays(dateObj.current,dateObj.endDate);
            return date;
        }
        //计算总价的函数
        ...
        $myHttpService.post("api/busline/queryCycleBusSchedulePrice",{
            ...
            $scope.orderInfo.month = date.getMonth()+1;
            //获得一个format格式的时间字符对象
            var dateObj = getMonthDate($scope.orderInfo.year, $scope.orderInfo.month);
            $scope.orderInfo.totalMoney = Math.floor(getTotalMoney(data.totalpay, dateObj.workDays, dateObj.remainDays));
            $scope.status=true;
        ...
        $scope.submitOrder  = function(){
            $myHttpService.post("api/busline/buyMonthTicket",{
                ...
                // timestr: dateObj.current + '&' + dateObj.endDate,
                // workDays: dateObj.reminDays + '&' + dateObj.workDays
            },function(data){
                function onBridgeReady(){
                    WeixinJSBridge.invoke(
                        'getBrandWCPayRequest',
