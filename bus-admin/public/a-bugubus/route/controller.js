 ...
 */
app.controller('RouteEditController',function($tableListService,$compile,$rootScope,$scope,$http,$state,$localStorage,$stateParams,$filter,$myHttpService){
    ...
    $scope.submit = function(saveMode){
        ...
        if(len>1){
            var stations = [];
            for(var i=0;i<len;i++){
                if($scope.buslineStations[i].stationName == '新建站点'){
                    layer.msg('请命名站点名称');
                    return;
                }
                stations.push({
